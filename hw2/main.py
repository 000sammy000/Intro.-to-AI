...
def train(model_type, model, train_dataloader, test_dataloader, optimizer, loss_fn, config):
    ...
    for epoch in range(config['epochs']):
      ...
      for batch,(X,y) in enumerate(train_dataloader): # This is pseudo code 
        ...
        optimizer.step()
        if (batch+1) % 100 == 0:
              print(f'Train Epoch: {epoch} ({100. * batch / len(train_dataloader):.0f}%)\tLoss: {loss.item():.6f}')
      ...
      with torch.no_grad():
          for X,y in test_dataloader: # This is pseudo code
              X,y=X.to(config['device']),y.to(config['device'])
              output=model.forward(X)

...
class YourModel(nn.Module):
    def __init__(
        ...
        self.rnn=nn.LSTM(
          input_size=embedding_dim, #每個詞彙變成固定dim，方向不一樣
          hidden_size=hidden_dim,
          num_layers=num_layer,
          dropout=dropout,
          
        )
        self.ln=nn.Linear(hidden_dim,output_dim)
    ...
    def forward(self, text):
        ...
        output=self.dropout(hidden[-1,:,:]) #最後一層包含整個訊息的狀態向量，防止overfitting

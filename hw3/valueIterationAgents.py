...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def runValueIteration(self):
        ...
        for i in range(self.iterations):
            iterValue=util.Counter()
            for state in self.mdp.getStates():
                if self.mdp.isTerminal(state):
                    self.values[state]=self.mdp.getReward(state, 'exit', '')
                else:
                    actions=self.mdp.getPossibleActions(state)
                    iterValue[state]=max([self.computeQValueFromValues(state,action) for action in actions])
    ...
    def computeQValueFromValues(self, state, action):
        ...
        result=0
        for t in tsap:
            transReward=self.mdp.getReward(state, action, t[0])
            result=result+transReward+self.discount*(self.values[t[0]]*t[1])
